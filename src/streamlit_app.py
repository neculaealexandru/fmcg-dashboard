...
COLORS = {"blue": "#3b82f6", "indigo": "#6366f1", "teal": "#14b8a6", "amber": "#f59e0b", "green": "#34d399", "grid": "#1e2d5a", "text": "#8892b0"}

def theme(fig):
    fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(family="Inter", color=COLORS["text"], size=12), title_font=dict(family="Space Grotesk", color="#e8eaf0", size=16), xaxis=dict(gridcolor=COLORS["grid"], linecolor=COLORS["grid"]), yaxis=dict(gridcolor=COLORS["grid"], linecolor=COLORS["grid"]), legend=dict(bgcolor="rgba(0,0,0,0)"), margin=dict(t=40, b=20, l=10, r=10))
    return fig
...
with c1:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=fmcg_market["year"], y=fmcg_market["market_size_bn_eur"], mode="lines", fill="tozeroy", line=dict(color=COLORS["blue"], width=2.5), fillcolor="rgba(59,130,246,0.10)", name="Total FMCG"))
    fig.add_trace(go.Scatter(x=fmcg_market["year"], y=fmcg_market["confectionery_bn_eur"], mode="lines+markers", line=dict(color=COLORS["amber"], width=2, dash="dot"), marker=dict(size=5), name="Confectionery"))
    fig.add_trace(go.Scatter(x=fmcg_market["year"], y=fmcg_market["beverages_bn_eur"], mode="lines+markers", line=dict(color=COLORS["teal"], width=2, dash="dot"), marker=dict(size=5), name="Beverages"))
...
with c2:
    ...
    fig2 = go.Figure(go.Pie(labels=["Confectionery", "Beverages", "Personal Care", "Other Food"], values=[row["confectionery_bn_eur"], row["beverages_bn_eur"], row["personal_care_bn_eur"], other], hole=0.55, marker_colors=[COLORS["amber"], COLORS["teal"], COLORS["indigo"], COLORS["blue"]]))
...
with c1:
    ...
    fig3.add_trace(go.Bar(x=consumer["year"], y=consumer["avg_monthly_spend_eur"], name="Monthly Spend (€)", marker_color=COLORS["blue"], opacity=0.8), secondary_y=False)
    fig3.add_trace(go.Scatter(x=consumer["year"], y=consumer["online_channel_pct"], name="Online %", mode="lines+markers", line=dict(color=COLORS["green"], width=2)), secondary_y=True)
    fig3.update_layout(title="Monthly Spend & Online Channel Growth", hovermode="x unified")
    fig3.update_yaxes(title_text="€ / month", secondary_y=False, gridcolor=COLORS["grid"])
...
with c2:
    fig4 = go.Figure()
    fig4.add_trace(go.Scatter(x=consumer["year"], y=consumer["private_label_pct"], mode="lines+markers", fill="tozeroy", line=dict(color=COLORS["indigo"], width=2), fillcolor="rgba(99,102,241,0.12)", name="Private Label %"))
    fig4.add_trace(go.Scatter(x=consumer["year"], y=consumer["consumer_confidence"], mode="lines", line=dict(color=COLORS["amber"], width=2, dash="dot"), name="Consumer Confidence", yaxis="y2"))
    fig4.update_layout(title="Private Label vs Consumer Confidence", yaxis=dict(title="Private Label %", gridcolor=COLORS["grid"]), yaxis2=dict(title="Confidence Index", overlaying="y", side="right", color=COLORS["amber"]), hovermode="x unified")
...
with r2:
    ...
    st.plotly_chart(theme(fig6), use_container_width=True)
